...
# This script proceses data from GeoIP-legacy.csv built by
...
for start_ip, end_ip, _, _, country_code, _ in csv.reader(
    line.decode('utf-8') for line in gzip.GzipFile(fileobj=io.BytesIO(response.content))
):

    if IpaddrType(start_ip) == "IPv4" :
